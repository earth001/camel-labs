...
//    private static Logger logger = LoggerFactory.getLogger(MyRouteBuilder.class);
	...
	public void configure() throws Exception {
		...
		from("timer://exercise?delay=2000&period=10s")
				...
				.setBody().constant(calculation)

				.setHeader("value1").simple("${body.getValue1}").setHeader("value2").simple("${body.getValue2}")

				// Set different Headers
				// using ognl & simple expressions
				.setHeader("addition").ognl("request.body.addition()").setHeader("subtraction")
				.ognl("request.body.subtraction()")
