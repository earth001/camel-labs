	...
	private static class MyRouteBuilder extends RouteBuilder {
		...
		public void configure() throws Exception {
			...
			from("timer://threadDirect?period=5s&delay=1s").routeId("# Timer for Direct #")
					.log(">> Timer Direct thread : ${threadName}").to("direct:thread");

			from("direct:thread").log(">> Direct thread : ${threadName}").to("direct:continueThread");

			from("direct:continueThread").log(">> Direct continue thread : ${threadName}");
